...
This defines the DOM element that you want to highlight and the message you want to show when the button is clicked. The idea is that if Beacon will only show the button if it can find the DOM element on the page.

## Additional Exploration

- This could easily evolve into a multi-step wizard that could be use not only for instructions but also for product onboarding.
...
This is incredibly hacky, but it should work for most cases.
